...
import CommerceCard from '../commerce/CommerceCard';
import { CommerceCardPayload } from '../commerce/NuomiCommerceMiniApp';
...
interface MessageItemProps {
    ...
    /** 购物中心 / 外卖卡片动作 */
    onCommerceCardAction?: (action: 'mark_paid' | 'mark_rejected', card: CommerceCardPayload) => void;
    /** 瑞幸菜单卡 (与麦当劳同构) */
...
const MessageItem = React.memo(({
    ...
    if (m.type === 'commerce_card') {
        const card = m.metadata?.commerceCard as CommerceCardPayload | undefined;
        if (!card) {
            return commonLayout(
                <div className="px-4 py-3 rounded-2xl bg-slate-50 text-slate-400 text-xs italic border border-slate-100">[购物/外卖卡片数据缺失]</div>
            );
        }
        return commonLayout(
            <CommerceCard card={card} onAction={onCommerceCardAction} />
        );
    }
