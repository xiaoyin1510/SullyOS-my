...
export default function NuomiCommerceMiniApp({ open, initialMode = 'shopping', onClose, char, userProfile, apiConfig, recentMessages, onSendToChat }: Props) {
    ...
    return <div className="fixed inset-0 z-[70] bg-slate-900/35 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-3" onMouseDown={onClose}>
        <section className="w-full sm:max-w-[980px] h-[100dvh] sm:h-[800px] sm:max-h-[92vh] rounded-none sm:rounded-[34px] bg-[#f8fafc] border border-white/70 shadow-2xl overflow-hidden flex flex-col" onMouseDown={e => e.stopPropagation()}>
            ...
            <div className="px-3 sm:px-4 py-3 shrink-0 bg-white border-b border-slate-100"><div className="flex flex-wrap gap-2"><button onClick={() => setApiOpen(true)} className="h-8 px-3 rounded-full text-xs font-black whitespace-nowrap bg-slate-100 text-slate-700 border border-slate-200">⚙ API</button><button onClick={() => setDataOpen(true)} className="h-8 px-3 rounded-full text-xs font-black whitespace-nowrap bg-sky-50 text-sky-600 border border-sky-100">数据</button><button onClick={doAi} disabled={loadingAi} className="h-8 px-3 rounded-full text-xs font-black whitespace-nowrap bg-pink-50 text-pink-500 border border-pink-100 flex items-center gap-1">{loadingAi ? <ArrowsClockwise className="w-3.5 h-3.5 animate-spin" /> : <Sparkle className="w-3.5 h-3.5" weight="fill" />}AI补货</button><button onClick={() => setManage(v => !v)} className="h-8 px-3 rounded-full text-xs font-black whitespace-nowrap bg-amber-50 text-amber-600 border border-amber-100">{manage ? '完成管理' : '管理'}</button></div></div>
            <div className="p-3 sm:p-4 flex-1 min-h-0 overflow-y-auto overscroll-contain">
                <main className="flex flex-col gap-3 overflow-visible">
                    <div className="sticky top-0 z-30 -mx-3 sm:-mx-4 px-3 sm:px-4 py-2 bg-[#f8fafc]/95 backdrop-blur border-b border-slate-100">
