...
function AddProductCard({ mode, category, onAdd }: { mode: CommerceMode; category: string; onAdd: () => void }) {
    ...
    return <button type="button" onClick={onAdd} className="min-h-[220px] rounded-3xl border-2 border-dashed border-pink-300 bg-white/55 text-pink-500 flex flex-col items-center justify-center gap-3 active:scale-95">
...
function ProductCard({ mode, product, checked, manage, selected, onCheck, onPick, onAdd, onDelete }: { mode: CommerceMode; product: Product; checked?: boolean; manage?: boolean; selected?: boolean; onCheck: () => void; onPick: () => void; onAdd: () => void; onDelete: () => void }) {
    return <article className={`relative rounded-3xl border p-3 bg-white/80 shadow-sm ${selected ? 'border-pink-300 ring-2 ring-pink-100' : 'border-slate-100'}`}>
