...
function ProductCard({ mode, product, checked, manage, selected, onCheck, onPick, onAdd, onDelete }: { mode: CommerceMode; product: Product; checked?: boolean; manage?: boolean; selected?: boolean; onCheck: () => void; onPick: () => void; onAdd: () => void; onDelete: () => void }) {
    return <article className={`relative rounded-2xl sm:rounded-3xl border p-1.5 sm:p-3 bg-white shadow-sm ${selected ? 'border-pink-300 ring-2 ring-pink-100' : 'border-slate-100'}`}>
        ...
        <button type="button" onClick={onPick} className="w-full text-left">
            ...
            <div className="min-w-0"><h4 className="text-[10px] sm:text-sm font-black text-slate-800 leading-tight line-clamp-2 min-h-[22px] sm:min-h-0">{product.name}</h4><p className="block text-[9px] sm:text-[11px] text-slate-400 font-bold mt-0.5 truncate">{product.source === 'manual' ? '自定义 · ' : product.source === 'ai' ? 'AI · ' : ''}{product.category}</p></div>
...
export default function NuomiCommerceMiniApp({ open, initialMode = 'shopping', onClose, char, userProfile, apiConfig, recentMessages, onSendToChat }: Props) {
    ...
    const requestFood = async () => { const items = deliveryItems(); if (!items[0]?.name || dtotal <= 0) return setToast('请先从外卖菜单里选择商品'); await sendCard(card('delivery_request', 'delivery', items, dtotal, deliveryNote), 'user', '外卖请求卡片已发送'); setDeliveryCart([]); setDeliveryNote(''); setToast('已发送外卖请求卡片'); };
