...
function AddProductCard({ mode, category, onAdd }: { mode: CommerceMode; category: string; onAdd: () => void }) {
    ...
    return <button type="button" onClick={onAdd} className="min-h-[106px] sm:min-h-[220px] rounded-2xl sm:rounded-3xl border-2 border-dashed border-pink-300 bg-white/70 text-pink-500 flex flex-col items-center justify-center gap-1.5 sm:gap-3 active:scale-95">
...
function ProductCard({ mode, product, checked, manage, selected, onCheck, onPick, onAdd, onDelete }: { mode: CommerceMode; product: Product; checked?: boolean; manage?: boolean; selected?: boolean; onCheck: () => void; onPick: () => void; onAdd: () => void; onDelete: () => void }) {
    return <article className={`relative rounded-2xl sm:rounded-3xl border p-1.5 sm:p-3 bg-white shadow-sm ${selected ? 'border-pink-300 ring-2 ring-pink-100' : 'border-slate-100'}`}>
        ...
        <button type="button" onClick={onPick} className="w-full text-left">
            <div className="aspect-square sm:aspect-[4/3] rounded-xl sm:rounded-2xl bg-gradient-to-br from-pink-50 to-orange-50 flex items-center justify-center overflow-hidden mb-1.5 sm:mb-3">{product.image ? <img src={product.image} alt="" className="w-full h-full object-cover" /> : <span className="text-2xl sm:text-4xl">{product.emoji || '🎁'}</span>}</div>
            <div className="min-w-0"><h4 className="text-[10px] sm:text-sm font-black text-slate-800 leading-tight line-clamp-2 min-h-[24px] sm:min-h-0">{product.name}</h4><p className="hidden sm:block text-[11px] text-slate-400 font-bold mt-0.5">{product.source === 'manual' ? '自定义 · ' : product.source === 'ai' ? 'AI · ' : ''}{product.category}</p></div>
            <strong className="mt-1 block text-[11px] sm:text-sm text-pink-500 shrink-0">{money(product.price)}</strong>
            {mode === 'shopping' && product.description && <p className="hidden sm:line-clamp-2 text-[12px] text-slate-500 leading-relaxed mt-2">{product.description}</p>}
        </button>
